...
// หน้าจอรายละเอียดเจ้าหนี้ — ทะเบียนเจ้าหนี้พร้อมที่อยู่แบบแยกช่อง
//
// จงใจให้เหมือนหน้ารายละเอียดลูกค้าทุกอย่าง เพราะเป็นคู่ค้าเหมือนกัน
// แค่คนละทิศทางของการค้า คนที่ใช้หน้านั้นเป็นแล้วจะใช้หน้านี้ได้ทันที
//
...
import { BRANCH_KINDS, SUPPLIER_KINDS } from "@/lib/constants";
import { nextSupCode } from "@/lib/db";
...
export default function Suppliers() {
  ...
  async function save() {
    ...
    if (!code) return toast("กรุณากรอกรหัสเจ้าหนี้", "err");
    if (!name) return toast("กรุณากรอกชื่อเจ้าหนี้", "err");

    // รหัสเจ้าหนี้เป็นตัวที่คนใช้อ้างถึงกันในเอกสาร ฐานข้อมูลก็มี unique index กันไว้
    // แต่ตรวจตรงนี้ก่อนเพื่อให้ข้อความบอกได้ว่าซ้ำกับใคร
    const dup = all.find((c) => c.id !== form.id && c.code.toUpperCase() === code);
    if (dup) return toast("รหัสเจ้าหนี้ " + code + " ถูกใช้โดย " + dup.name + " แล้ว", "err");

    const postcode = String(form.postcode || "").trim();
    if (postcode && !/^\d{5}$/.test(postcode)) return toast("รหัสไปรษณีย์ต้องเป็นตัวเลข 5 หลัก", "err");

    // เลขผู้เสียภาษีไทยมี 13 หลักเสมอ เว้นว่างได้ (ไม่จดทะเบียนภาษี) แต่กรอกแล้วต้องครบ
    // ภาษีซื้อที่เลขไม่ครบใช้ยื่นไม่ได้ และกว่าจะรู้ก็ตอนสรรพากรตีเอกสารกลับ
    ...
    try {
      ...
      toast("บันทึกเจ้าหนี้ " + name + " แล้ว", "ok");
      setForm(null);
    } catch (e) {
  ...
  async function remove(c) {
    if (busy) return;
    if (!window.confirm("ยืนยันการลบเจ้าหนี้ " + c.name + " ?")) return;
    setBusy(true);
    try {
      await inv.removeSupplier(c.id);
      toast("ลบเจ้าหนี้ " + c.name + " แล้ว", "ok");
    } catch (e) {
  ...
  if (!inv.suppliersReady) {
    return <SetupNotice feature="หน้าจอรายละเอียดเจ้าหนี้" tables={["suppliers"]} />;
  }
  ...
  return (
    <div className="stack">
        ...
        actions={
          <>
            ...
            <button
              ...
              onClick={() => setForm(blank(nextSupCode(db)))}
              disabled={!perm.edit}
            >
              <IcPlus size={15} />
              เพิ่มเจ้าหนี้
      ...
      >
        <div className="row" style={{ marginBottom: 12 }}>
          ...
          <select
            ...
            aria-label="กรองตามประเภทเจ้าหนี้"
          >
        ...
        {rows.length ? (
          <TableWrap>
            <thead>
              <tr>
                ...
                <th style={{ minWidth: 140 }}>ประเภทเจ้าหนี้</th>
                <th style={{ width: 150 }} />
              </tr>
            </thead>
            <tbody>
          ...
          <Empty>
            {all.length ? "ไม่พบเจ้าหนี้ที่ตรงกับที่ค้นหา" : "ยังไม่มีข้อมูลเจ้าหนี้ กดปุ่ม “เพิ่มเจ้าหนี้” เพื่อเริ่มต้น"}
          </Empty>
        )}
      </Card>
      ...
      {form ? (
        <Modal
          title={isNew ? "เพิ่มเจ้าหนี้" : "แก้ไขเจ้าหนี้ " + form.code}
        ...
        >
          <div className="form-grid" style={{ gridTemplateColumns: "repeat(2,1fr)" }}>
            <div className="field">
              <label className="lbl" htmlFor="sf_code">รหัสเจ้าหนี้</label>
              <input
                className="inp"
                id="sf_code"
                value={form.code}
            ...
            <div className="field">
              <label className="lbl" htmlFor="sf_kind">ประเภทเจ้าหนี้</label>
              <select
                className="sel"
                id="sf_kind"
                value={form.kind}
            ...
            <div className="field span2">
              <label className="lbl" htmlFor="sf_name">ชื่อเจ้าหนี้</label>
              <input
                className="inp"
                id="sf_name"
                value={form.name}
                onChange={(e) => set("name", e.target.value)}
                placeholder="ชื่อบริษัท ห้างร้าน หรือผู้ให้บริการ"
              />
            </div>

            <div className="field span2">
              <label className="lbl" htmlFor="sf_addr">ที่อยู่ (บ้านเลขที่ หมู่ ถนน)</label>
              <input
                className="inp"
                id="sf_addr"
                value={form.address}
            ...
            {addr ? (
              <>
                ...
                <div className="field">
                  <label className="lbl" htmlFor="sf_post">รหัสไปรษณีย์</label>
                  {autoPost ? (
                    /* ตรงกับตำบลที่เลือกแล้ว แก้เองไม่ได้ รหัสที่ไม่ตรงคือของส่งไม่ถึง */
                    <input className="inp" id="sf_post" value={form.postcode} readOnly />
                  ) : (
                    <input
                      className="inp"
                      id="sf_post"
              ...
              <>
                ...
                <div className="field">
                  <label className="lbl" htmlFor="sf_dis">อำเภอ / เขต</label>
                  <input
                    className="inp"
                    id="sf_dis"
                    value={form.district}
                ...
                <div className="field">
                  <label className="lbl" htmlFor="sf_sub">ตำบล / แขวง</label>
                  <input
                    className="inp"
                    id="sf_sub"
                    value={form.subdistrict}
            ...
            <div className="field">
              <label className="lbl" htmlFor="sf_tax">เลขประจำตัวผู้เสียภาษี (13 หลัก)</label>
              <input
                className="inp"
                id="sf_tax"
                value={form.taxId || ""}
                ...
                placeholder="ไม่จดทะเบียนภาษีเว้นว่างได้"
              />
            </div>
            <div className="field">
              <label className="lbl" htmlFor="sf_branch">สำนักงานใหญ่ / สาขา</label>
              <input
                className="inp"
                id="sf_branch"
                list="sf_branch_list"
            ...
            <div className="field span2">
              <label className="lbl" htmlFor="sf_phone">เบอร์โทร</label>
              <input
                className="inp"
                id="sf_phone"
                value={form.phone}
