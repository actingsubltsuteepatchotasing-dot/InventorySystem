...
/* ----------------------------------------------------------------- 22 */
head("22. ลูกค้าของบิลขายหน้าร้านต่อครบทั้งสาย");
{
  /*
   * สายนี้ขาดตรงไหนก็เงียบทั้งนั้น ไม่มีใครฟ้อง:
   *   เลือกรหัสลูกค้าที่หน้า POS ได้ แต่รหัสไม่ได้ลงบิล -> รายงานรายลูกค้าว่างเปล่า
   *   ตั้งลูกค้าเริ่มต้นได้ แต่หน้า POS ไม่ได้อ่าน      -> กดแล้วไม่มีอะไรเกิดขึ้น
   *   ฟอร์มมีช่องให้กรอก แต่ตอนบันทึกไม่ได้ส่งค่าไป    -> กรอกแล้วหาย (เคยเกิดกับพนักงานขายประจำ)
   * จึงไล่ตรวจทีละข้อต่อทั้งเส้น ตั้งแต่หน้าจอไปจนถึงคำสั่งบันทึกบนฐานข้อมูล
   */
  const pos = read("components/views/POS.js");
  const custScreen = read("components/views/Customers.js");
  const api = read("lib/api.js");
  const dbSrc = read("lib/db.js");
  const schema = read("supabase/schema.sql");
  let n = 0;

  // ---- หน้า POS เลือกรหัสลูกค้าจากทะเบียนได้ และชื่อวิ่งตามรหัส
  if (!/id="pos_cust_code"/.test(pos) || !/SearchSelect/.test(pos)) {
    bad("หน้า POS ไม่มีช่องเลือกรหัสลูกค้าแบบค้นหาได้");
    n++;
  }
  if (!/id="pos_cust"[^>]*readOnly/.test(pos)) {
    bad(
      "หน้า POS ไม่ได้ล็อกช่องชื่อลูกค้าตอนเลือกรหัสไว้ — " +
        "จะได้บิลที่รหัสกับชื่อเป็นคนละคน แล้วรายงานจะแยกไม่ออกว่าเชื่อช่องไหน"
    );
    n++;
  }

  // ---- รหัสที่เลือกต้องลงไปถึงบิลจริง ไม่ใช่แค่โชว์บนหน้าจอ
  ["customerId", "custCode"].forEach((k) => {
    if (!new RegExp(k + ":").test(pos)) {
      bad("หน้า POS ไม่ได้ใส่ " + k + " ลงในบิล — เลือกรหัสแล้วรหัสไม่ถูกบันทึก");
      n++;
    }
  });
  if (!/customer_id: s\.customerId/.test(api) || !/cust_code: s\.custCode/.test(api)) {
    bad("lib/api.js ไม่ได้แปลงรหัสลูกค้าของบิลขายหน้าร้านตอนเขียนลงฐานข้อมูล");
    n++;
  }
  // create_sale เป็นตัวเขียนจริง ไม่ได้เขียนผ่าน REST ตรง ๆ ลืมตรงนี้แล้วค่าหายทั้งที่แปลงครบ
  const saleInsert = /insert into public\.sales \(([\s\S]*?)\)/.exec(schema);
  if (!saleInsert || !/customer_id/.test(saleInsert[1]) || !/cust_code/.test(saleInsert[1])) {
    bad("ฟังก์ชัน create_sale ไม่ได้เขียนรหัสลูกค้าลงตาราง sales");
    n++;
  }

  // ---- ลูกค้าเริ่มต้นของหน้า POS
  if (!/export function posCustomerOf/.test(dbSrc)) {
    bad("lib/db.js ไม่มี posCustomerOf — ไม่มีที่เดียวที่ตัดสินว่าใครเป็นลูกค้าเริ่มต้น");
    n++;
  }
  ["components/views/POS.js", "components/views/Customers.js"].forEach((f) => {
    if (!/posCustomerOf/.test(read(f))) {
      bad(f + " ไม่ได้ใช้ posCustomerOf — สองหน้าจะตอบไม่ตรงกันว่าใครเป็นค่าเริ่มต้น");
      n++;
    }
  });
  if (!/set\("posDefault"/.test(custScreen)) {
    bad("หน้ารายละเอียดลูกค้าไม่มีปุ่มตั้งเป็นลูกค้าเริ่มต้นของหน้า POS");
    n++;
  }
  if (!/pos_default: !!c\.posDefault/.test(api)) {
    bad("lib/api.js ไม่ได้เขียนธงลูกค้าเริ่มต้นลงฐานข้อมูล — กดตั้งแล้วไม่ติด");
    n++;
  }
  // ตั้งได้ทีละรายเดียว ต้องปลดของรายอื่นก่อนบันทึก ไม่งั้นเปิดหน้า POS ได้ลูกค้าคนละรายสลับกัน
  if (!/pos_default=is\.true[\s\S]{0,400}pos_default: false/.test(api)) {
    bad("upsertCustomer ไม่ได้ปลดลูกค้าเริ่มต้นรายเดิมก่อนบันทึกรายใหม่");
    n++;
  }
  if (!/add column if not exists pos_default/.test(schema)) {
    bad("schema.sql ไม่มีคอลัมน์ pos_default ของตารางลูกค้า");
    n++;
  }

  /*
   * ---- ทุกช่องที่ฟอร์มลูกค้าแก้ได้ ต้องถูกส่งไปตอนบันทึกจริง
   * เคยหลุดมาแล้วกับ "พนักงานขายประจำ" — ฟอร์มมีช่องให้เลือก เลือกได้ปกติ
   * แต่ตอนบันทึกไม่ได้ส่งค่านั้นไป ค่าที่เลือกจึงหายทุกครั้งโดยไม่มีข้อความเตือนอะไรเลย
   */
  const saveArg = /inv\.saveCustomer\(\{([\s\S]*?)\n      \}\)/.exec(custScreen);
  if (!saveArg) {
    bad("อ่านรายการค่าที่หน้ารายละเอียดลูกค้าส่งตอนบันทึกไม่ออก");
    n++;
  } else {
    const keys = [...custScreen.matchAll(/\bset\("([A-Za-z]+)"/g)].map((m) => m[1]);
    [...new Set(keys)].forEach((k) => {
      // ส่งได้สองแบบ: เขียนเต็ม (kind: form.kind) หรือย่อจากตัวแปรชื่อเดียวกัน (code,)
      if (!new RegExp("(^|[\\s{])" + k + "\\s*[:,]").test(saveArg[1])) {
        bad("ฟอร์มลูกค้าแก้ " + k + " ได้ แต่ตอนบันทึกไม่ได้ส่งค่านี้ไป — กรอกแล้วหาย");
        n++;
      }
    });
  }
  ...
  if (!n) {
    ok(
      "ลูกค้าของบิลขายหน้าร้านต่อครบ · เลือกรหัสได้และชื่อวิ่งตามรหัส · " +
        "รหัสลงถึงตาราง sales จริง · ลูกค้าเริ่มต้นตั้งได้ทีละรายและสองหน้าจออ่านที่เดียวกัน · " +
        "ทุกช่องในฟอร์มลูกค้าถูกส่งตอนบันทึก"
    );
  }
}

/* ----------------------------------------------------------------- 23 */
head("23. ยอดคงเหลือรายช่องเก็บคิดที่เดียว ทุกหน้าตอบตรงกัน");
{
  /*
   * หน้ารายละเอียดสินค้ากับฟอร์มแก้ไขสินค้าแสดงยอดชุดเดียวกัน
   * ถ้าต่างฝ่ายต่างคำนวณเอง วันที่แก้กติกาการนับจะมีที่หนึ่งที่ลืมแก้
   * แล้วสองหน้าจะบอกยอดของสินค้าตัวเดียวกันไม่ตรงกัน ซึ่งคนใช้จับได้แน่
   * และไม่มีทางรู้ว่าควรเชื่อหน้าไหน
   */
  const dbSrc = read("lib/db.js");
  const storeSrc = read("lib/store.js");
  const bins = read("components/views/StockBins.js");
  let n = 0;

  if (!/export function stockByBin/.test(dbSrc)) {
    bad("lib/db.js ไม่มี stockByBin — ไม่มีที่เดียวที่คิดยอดรายช่องเก็บ");
    n++;
  }
  if (!/stockByBin: \(pid\)/.test(storeSrc)) {
    bad("store ไม่ได้แจก stockByBin ให้หน้าจอ");
    n++;
  }

  ["components/views/ProductDetail.js", "components/views/ProductForm.js"].forEach((f) => {
    const src = read(f);
    if (!/from "\.\/StockBins"/.test(src) || !/<StockBins/.test(src)) {
      bad(f + " ไม่ได้ใช้ตารางยอดคงเหลือรายช่องเก็บตัวกลาง");
      n++;
    }
    if (!/inv\.stockByBin\(/.test(src)) {
      bad(f + " ไม่ได้เรียก stockByBin — อาจกลับไปคิดยอดเอง");
      n++;
    }
    // คิดเองซ้ำอีกชุดในหน้าจอ = จุดเริ่มของการที่สองหน้าตอบไม่ตรงกัน
    if (/inv\.placedIn\(/.test(src)) {
      bad(f + " ยังไล่นับของในช่องเก็บเอง ทั้งที่มี stockByBin แล้ว");
      n++;
    }
  });

  // ผลรวมของช่องต้องเท่ากับยอดของคลังเสมอ ส่วนที่ไม่รู้ว่าอยู่ช่องไหนจึงต้องมีที่ยืน
  if (!/loose/.test(bins)) {
    bad(
      "ตารางยอดคงเหลือไม่ได้แสดงส่วนที่ยังไม่ระบุที่เก็บ — " +
        "ผลรวมของช่องจะไม่เท่ากับยอดของคลัง แล้วคนอ่านจะคิดว่าตัวเลขผิด"
    );
    n++;
  }

  if (!n) {
    ok("ยอดคงเหลือรายช่องเก็บคิดที่ lib/db.js ที่เดียว · สองหน้าจอสินค้าใช้ตารางตัวเดียวกัน · แสดงส่วนที่ยังไม่ระบุที่เก็บด้วย");
  }
}

/* ----------------------------------------------------------------- 24 */
head("24. สรุปยอดคงเหลือรายคลังบวกลงเสมอ");
{
  /*
   * ตารางหน้านี้ต้องบวกลงทุกแถว: ยกมา + รับเข้า − จ่ายออก = คงเหลือ
   * ตารางการเงินที่บวกไม่ลงคือตารางที่ใช้ไม่ได้เลย คนอ่านจะเลิกเชื่อทั้งหน้า
   * และไม่มีทางรู้ว่าตัวเลขไหนผิด จึงเฝ้าโครงที่ทำให้มันบวกลงไว้ตรงนี้
   */
  const src = read("lib/whSummary.js");
  const view = read("components/views/WarehouseSummary.js");
  let n = 0;

  ["whMovement", "whSummary", "whTotals", "whProducts", "whByMonth"].forEach((fn) => {
    if (!new RegExp("export function " + fn + "\\b").test(src)) {
      bad("lib/whSummary.js ไม่มี " + fn);
      n++;
    }
  });

  // ยอดคงเหลือต้องมาจาก stockMap ตัวเดียวกับที่ทั้งระบบใช้ ไม่ใช่ไล่นับเองอีกชุด
  if (!/import \{[^}]*stockMap[^}]*\} from "\.\/db"/.test(src)) {
    bad("lib/whSummary.js ไม่ได้ใช้ stockMap ของ lib/db.js — ยอดคงเหลือจะไม่ตรงกับหน้าอื่น");
    n++;
  }

  /*
   * ยกมาต้องเป็นค่าที่ "คำนวณย้อนจากคงเหลือ" ไม่ใช่ไล่นับใหม่อีกชุด
   * ถ้าไล่นับแยก วันที่กติกาการนับเปลี่ยนจะมีที่หนึ่งที่ลืมแก้ แล้วตารางจะบวกไม่ลง
   */
  if (!/open: close - m\.net/.test(src)) {
    bad("ยอดยกมาไม่ได้คิดย้อนจากยอดคงเหลือ — ตารางจะบวกไม่ลงเมื่อกติกาการนับเปลี่ยน");
    n++;
  }

  /*
   * ทุกช่องที่นับไว้ต้องอยู่ในแถวรวมด้วย
   * เพิ่มช่องใหม่แล้วลืมใส่ในแถวรวม = แถวรวมขึ้น NaN หรือหายไปเงียบ ๆ ทั้งคอลัมน์
   */
  const fields = [...(/const blank = \(\) => \(\{([\s\S]*?)\}\)/.exec(src) || ["", ""])[1]
    .matchAll(/(\w+):\s*0/g)].map((m) => m[1]);
  const totalKeys = (/export function whTotals[\s\S]*?const keys = \[([\s\S]*?)\];/.exec(src) || ["", ""])[1];
  fields.forEach((f) => {
    if (!new RegExp('"' + f + '"').test(totalKeys)) {
      bad("แถวรวมทุกคลังไม่ได้รวมช่อง " + f + " — คอลัมน์นั้นจะหายไปจากบรรทัดสรุป");
      n++;
    }
  });
  if (!fields.length) {
    bad("อ่านรายชื่อช่องที่นับใน lib/whSummary.js ไม่ออก");
    n++;
  }

  // หน้าจอต้องอ่านตัวเลขจากตัวคำนวณตัวนี้ ไม่ใช่คิดเองซ้ำอีกชุด
  ["whSummary", "whTotals"].forEach((fn) => {
    if (!new RegExp(fn + "\\(").test(view)) {
      bad("หน้าสรุปยอดคงเหลือรายคลังไม่ได้เรียก " + fn);
      n++;
    }
  });
  if (/stockMap\(/.test(view)) {
    bad("หน้าสรุปยอดคงเหลือรายคลังไล่นับยอดเอง ทั้งที่ควรอ่านจาก lib/whSummary.js");
    n++;
  }
...
head("26. คู่มือติดตั้งตรงกับของจริง");
{
  ...
  /*
   * ---- เช็คลิสต์ Excel ต้องมีตัวสร้าง และตัวสร้างต้องอธิบายตัวแปรครบ
   * ไฟล์ .xlsx เป็นไฟล์ไบนารี ตรวจเนื้อในจากที่นี่ไม่ได้ จึงตรวจที่ตัวสร้างแทน
   * ตัวสร้างมีด่านของตัวเองอยู่แล้ว (หยุดทำงานถ้าอ้างหน้าจอที่ไม่มีจริง
   * หรือเจอตัวแปรที่ไม่ได้อธิบาย) ตรงนี้แค่กันไม่ให้ด่านนั้นถูกถอดออก
   */
  /*
   * ---- คำสั่งทุกคำสั่งที่มีจริง ต้องถูกอธิบายไว้
   * เพิ่ม script หรือเพิ่มไฟล์ใน tools/ แล้วไม่มีใครรู้ว่ามันมี ก็เท่ากับไม่มี
   */
  const pkg = JSON.parse(read("package.json"));
  Object.keys(pkg.scripts || {}).forEach((k) => {
    if (!guide.includes("npm run " + k)) {
      bad("คู่มือติดตั้งไม่ได้อธิบายคำสั่ง npm run " + k);
      n++;
    }
  });
  fs.readdirSync(path.join(ROOT, "tools")).forEach((f) => {
    if (!guide.includes("tools/" + f)) {
      bad("คู่มือติดตั้งไม่ได้อธิบายสคริปต์ tools/" + f);
      n++;
    }
  });

  /*
   * ---- หัวข้อความปลอดภัยที่ขาดไม่ได้
   * ทุกข้อในนี้คือเรื่องที่พลาดแล้วเสียหายจริงและกู้กลับยาก
   * ข้อ "สิทธิหน้าจอไม่ใช่กำแพงกันข้อมูล" สำคัญที่สุด เพราะถ้าคนเข้าใจผิด
   * จะให้บัญชีระบบกับคนที่ไม่ควรเห็นข้อมูลทั้งหมด โดยคิดว่าตั้งสิทธิแล้วปลอดภัย
   */
  [
    ["service_role", "ต้องเตือนว่ากุญแจนี้ห้ามออกจากฝั่งเซิร์ฟเวอร์"],
    ["ไม่ใช่กำแพงกันข้อมูล", "ต้องบอกตรง ๆ ว่าสิทธิหน้าจอคุมลำดับงาน ไม่ได้กั้นข้อมูลระดับ API"],
    ["Allow new users to sign up", "ต้องบอกให้ปิดการสมัครเองเมื่อสร้างผู้ใช้ครบแล้ว"],
    ["ไฟล์สำรองที่กู้ไม่ได้", "ต้องบอกให้ทดลองกู้คืนจริงก่อนใช้งาน"],
  ].forEach(([word, why]) => {
    if (!guide.includes(word)) {
      bad("หัวข้อความปลอดภัยในคู่มือขาดเรื่อง " + word + " — " + why);
      n++;
    }
  });

  /*
   * ---- คู่มือต้องอธิบายตัวช่วยติดตั้งให้ตรงกับที่มันตรวจจริง
   *
   * ตัวช่วยติดตั้งเป็นด่านแรกที่คนใช้ตอนติดตั้ง ถ้าคู่มืออธิบายไม่ตรง
   * คนจะเชื่อผลของมันผิด ๆ โดยเฉพาะข้อที่มันตรวจไม่ได้
   */
  /*
   * ---- คู่มือต้องมีผังลำดับงาน ไม่ใช่ข้อความล้วน
   *
   * ขั้นตอนติดตั้งเป็นเรื่องของ "อะไรต้องมาก่อนอะไร" ซึ่งอ่านจากข้อความยาว ๆ
   * แล้วนึกภาพไม่ออก คนจึงทำ Vercel ก่อน Supabase แล้วไปติดตรงที่ไม่มีค่าจะใส่
   * ผังทำให้เห็นความสัมพันธ์ในภาพเดียว · ใช้อักขระวาดเส้นธรรมดา
   * จึงอ่านได้เหมือนกันทั้งบน GitHub และในโปรแกรมแก้ไขข้อความ
   */
  if (!/^## ภาพรวม/m.test(guide)) {
    bad("คู่มือติดตั้งไม่มีหัวข้อภาพรวมที่บอกว่าระบบประกอบด้วยอะไร");
    n++;
  }
  const blocks = [...guide.matchAll(/```[a-z]*\n([\s\S]*?)```/g)].map((m) => m[1]);
  const flows = blocks.filter((b) => /\n\s*v\n/.test(b) || /\+--/.test(b) || /-->/.test(b));
  if (flows.length < 6) {
    bad("คู่มือติดตั้งมีผังลำดับงานแค่ " + flows.length + " ผัง — ควรมีอย่างน้อย 6 ผัง");
    n++;
  }

  const doctor = read("tools/setup.mjs");
  if (!guide.includes("## ตัวช่วยติดตั้ง")) {
    bad("คู่มือติดตั้งไม่มีหัวข้ออธิบายตัวช่วยติดตั้ง");
    n++;
  }
  // ทุกหมวดที่มันตรวจ ต้องมีในตารางของคู่มือ ไม่งั้นคนอ่านจะไม่รู้ว่ามันดูอะไรให้บ้าง
  [...doctor.matchAll(/head\("\d+\. ([^"]+)"\)/g)].map((m) => m[1]).forEach((h) => {
    if (!guide.includes(h)) {
      bad("คู่มือไม่ได้บอกว่าตัวช่วยติดตั้งตรวจหมวด " + h);
      n++;
    }
  });
  /*
   * ต้องบอกข้อจำกัดด้วย — มันอ่านค่าจากเครื่องที่รัน ไม่ได้อ่านจาก Vercel
   * ไม่บอกไว้ คนจะเห็นว่า "พร้อมใช้งาน" แล้วงงว่าทำไมเว็บจริงยังล็อกอินไม่ได้
   */
  if (!/ไม่ได้ตรวจค่าที่ตั้งไว้บน Vercel/.test(guide)) {
    bad("คู่มือไม่ได้บอกว่าตัวช่วยติดตั้งตรวจค่าบน Vercel ไม่ได้ — คนจะเชื่อผลผิด");
    n++;
  }

  const gen = "tools/make-setup-xlsx.mjs";
  if (!fs.existsSync(path.join(ROOT, gen))) {
    bad("ไม่มี " + gen + " — เช็คลิสต์ Excel จะสร้างใหม่ไม่ได้เมื่อระบบเปลี่ยน");
    n++;
  } else {
    const src = read(gen);
    if (!/throw new Error/.test(src) || !/ENV_DESC\[v\]/.test(src)) {
      bad(gen + " ไม่มีด่านกันเนื้อหาเพี้ยน — ต้องหยุดทำงานเมื่อเจอตัวแปรที่ไม่ได้อธิบาย");
      n++;
    }
    envVars.forEach((v) => {
      if (!src.includes(v)) {
        bad("เช็คลิสต์ Excel ไม่ได้พูดถึงตัวแปร " + v);
        n++;
      }
    });
    if (!fs.existsSync(path.join(ROOT, "Docs/เช็คลิสต์การติดตั้ง.xlsx"))) {
      bad("ยังไม่ได้สร้าง Docs/เช็คลิสต์การติดตั้ง.xlsx — สั่ง node " + gen);
      n++;
    }
  }
  ...
  if (!n) {
    ok(
      "คู่มือติดตั้งตรงกับของจริง · ตัวแปรครบ " + envVars.length + " ตัวในไฟล์ตัวอย่างและเช็คลิสต์ · " +
