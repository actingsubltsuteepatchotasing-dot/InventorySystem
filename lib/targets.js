...
/**
 * ค่าที่เคยใช้ของมิติหนึ่งในทะเบียนสินค้า
 * ใช้ทำรายการให้เลือกโดยไม่ต้องมีหน้าจอจัดการมิติแยกอีกหน้า
 */
export const valuesOf = (db, field) =>
  [...new Set((db.products || []).map((p) => p[field]).filter(Boolean))].sort((a, b) =>
