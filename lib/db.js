...
import { localISO, rng, todayISO, uid } from "./format";
...
export function seed() {
  const db = {
    ...
    invoices: [],
    invoiceItems: [],
    suppliers: SEED_SUP.map((x) => ({ ...x })),
    // ใบซื้อและใบส่งคืนไม่มีตัวอย่างให้ ด้วยเหตุผลเดียวกับใบขาย — แต่ละใบขยับสต็อกจริง
    purchases: [],
    purchaseItems: [],
    purchaseReturns: [],
    purchaseReturnItems: [],
...
/**
 * เลขที่ใบเสนอราคาถัดไป เช่น QT-202609-0007
 *
 * นับจากตารางใบเสนอราคาเอง ไม่ใช่จาก txns เหมือนเอกสารชนิดอื่น
 * เพราะใบเสนอราคาไม่ขยับสต็อก จึงไม่มีแถวใน txns ให้นับ
 * ถ้าไปนับจาก txns เลขจะเริ่มที่หนึ่งใหม่ทุกใบ แล้วเลขซ้ำกันทั้งหมด
 * (ฐานข้อมูลตั้ง unique ไว้ ใบที่สองจึงบันทึกไม่ผ่าน)
 */
export function nextQuoteNo(db, dateISO) {
  const group = docGroupOf(db, "QUOTE");
  const prefix = docPrefixOf(group, dateISO);
  const digits = docDigits(group);

  let max = 0;
  (db.quotes || []).forEach((q) => {
    if (!q.docNo || !q.docNo.startsWith(prefix)) return;
    const tail = q.docNo.slice(prefix.length);
    if (!/^\d+$/.test(tail)) return;
    const n = parseInt(tail, 10);
    if (n > max) max = n;
  });
...
/** รหัสเจ้าหนี้ถัดไปในรูปแบบ Vnnnn */
export function nextSupCode(db) {
  let n = 1;
  (db.suppliers || []).forEach((c) => {
    const m = /^V(\d+)$/.exec(c.code || "");
    if (m) n = Math.max(n, parseInt(m[1], 10) + 1);
  });
...
/**
 * ลูกค้าเริ่มต้นของหน้าขายสินค้า (POS) — null ถ้ายังไม่ได้ตั้งไว้
 *
 * ตั้งได้ทีละรายเดียว ตัวโปรแกรมปลดของรายอื่นให้เองตอนกดตั้ง (upsertCustomer)
 * แต่ที่นี่ยังเลือกรายที่รหัสน้อยสุดไว้กันเหนียว เผื่อไฟล์สำรองเก่าหรือการแก้ผ่าน
 * ฐานข้อมูลตรง ๆ ทำให้มีสองรายติ๊กไว้พร้อมกัน จะได้ไม่เปิดหน้า POS ทีได้ลูกค้าคนละราย
 * ตามลำดับที่ฐานข้อมูลคืนมา ซึ่งเป็นอาการที่ไล่หาสาเหตุแทบไม่ได้
 */
export function posCustomerOf(db) {
  const hit = (db.customers || []).filter((c) => c.posDefault);
  if (!hit.length) return null;
  return hit.slice().sort((a, b) => String(a.code).localeCompare(String(b.code), "th"))[0];
}

/* --------------------------------------------- สิทธิการใช้งานหน้าจอ */

/** สิทธิเต็มทุกอย่าง ใช้เป็นค่าตั้งต้นเมื่อยังไม่เคยตั้งสิทธิ */
const FULL_PERM = (screenId) => ({ id: screenId, view: true, edit: true, date: true });

/**
 * สิทธิของหน้าจอหนึ่ง สำหรับผู้ใช้คนหนึ่ง
 *
 * ตัดสินตามลำดับ หยุดที่ข้อแรกที่เจอ:
 *   1. สิทธิเฉพาะตัวของคนนั้น (user_perms)  — แอดมินตั้งให้เป็นรายคน
 *   2. ค่าเริ่มต้นของทุกคน (screen_perms)   — ตั้งครั้งเดียวมีผลกับคนที่ไม่มีสิทธิเฉพาะตัว
 *   3. ไม่มีทั้งคู่ = เปิดหมด
 *
 * ทำไมข้อ 3 เป็น "เปิดหมด" ไม่ใช่ "ปิดหมด":
 *   ถ้าค่าเริ่มต้นเป็นปิด ระบบที่เพิ่งติดตั้งจะเปิดมาแล้วว่างเปล่า
 *   คนใช้จะคิดว่าโปรแกรมพัง ไม่ได้คิดว่ายังไม่ได้ตั้งสิทธิ
 *
 * ทำไมข้อ 1 ไม่ใช่การ "รวม" กับข้อ 2:
 *   สิทธิที่ต้องเอาสองชั้นมารวมกันแล้วเดาว่าชั้นไหนชนะ เป็นสิ่งที่คนตั้งค่าอ่านไม่ออก
 *   มีสิทธิเฉพาะตัวเมื่อไร ให้ถือว่าคนนั้นถูกกำหนดไว้ชัดแล้ว ใช้ของคนนั้นล้วน ๆ
 *   หน้าจอกำหนดสิทธิจึงบอกได้ตรง ๆ ว่าคนนี้ "ใช้ค่าเริ่มต้น" หรือ "ตั้งเฉพาะตัวไว้"
 *
 * @param {object} db ข้อมูลทั้งกอง
 * @param {string} screenId รหัสหน้าจอ
 * @param {string} [userId] รหัสผู้ใช้ที่ล็อกอินอยู่ ไม่ส่งมา = ดูเฉพาะค่าเริ่มต้น
 */
export function permOf(db, screenId, userId) {
  if (!db) return FULL_PERM(screenId);

  if (userId) {
    const own = (db.userPerms || []).find(
      (p) => p.userId === userId && p.screenId === screenId
    );
    if (own) return { id: screenId, view: own.view, edit: own.edit, date: own.date };
  }

  const found = (db.perms || []).find((p) => p.id === screenId);
  return found || FULL_PERM(screenId);
}

/**
 * ผู้ใช้คนนี้เป็นแอดมินหรือไม่
 *
 * ยังไม่มีแอดมินเลยสักคน = ทุกคนเป็นแอดมิน
 ...
 *   พอตั้งแอดมินคนแรกแล้ว ประตูนี้ปิดเองทันที
 *   กติกาเดียวกันนี้เขียนไว้ในฟังก์ชัน is_admin() ของฐานข้อมูลด้วย
 *   ทั้งสองที่จึงตัดสินตรงกันเสมอ (ฝั่งเว็บไว้ซ่อนปุ่ม ฝั่งฐานข้อมูลไว้บังคับจริง)
 *
 * ยังไม่ได้สร้างตารางผู้ใช้ = ถือว่าเป็นแอดมิน เพื่อให้ระบบเดิมใช้งานต่อได้เหมือนเดิม
 */
export function isAdminOf(db, userId) {
  const users = (db && db.appUsers) || [];
  if (!users.length) return true;
  if (!users.some((u) => u.role === "admin" && u.active)) return true;
  return users.some((u) => u.id === userId && u.role === "admin" && u.active);
}

/** แถวผู้ใช้ของคนที่ล็อกอินอยู่ ใช้ดูบทบาทและสถานะของตัวเอง */
export function appUserOf(db, userId) {
  return ((db && db.appUsers) || []).find((u) => u.id === userId) || null;
}

/** จำนวนหน้าจอที่คนนี้ถูกตั้งสิทธิเฉพาะตัวไว้ */
export function userPermCount(db, userId) {
  return ((db && db.userPerms) || []).filter((p) => p.userId === userId).length;
}
...
/** รายการในใบซื้อใบหนึ่ง */
export function itemsOfPurchase(db, purchaseId) {
  return (db.purchaseItems || [])
    .filter((i) => i.purchaseId === purchaseId)
    .sort((a, b) => (a.seq || 0) - (b.seq || 0));
}

/** รายการในใบตรวจนับใบหนึ่ง เรียงตามลำดับที่เตรียมไว้ */
export function itemsOfCount(db, countId) {
  return (db.stockCountItems || [])
...
/** เหตุการณ์เดินสถานะของใบหนึ่ง เรียงตามเวลาเก่าไปใหม่ */
export function shipEventsOf(db, invoiceId) {
  return (db.shipEvents || [])
    .filter((e) => e.invoiceId === invoiceId)
    .sort((a, b) => a.ts - b.ts);
}

/**
 * เวลาที่ใบหนึ่งเข้าแต่ละสถานะ และเวลาที่ใช้ในแต่ละช่วง
 *
 * ใช้ "ครั้งแรกที่เข้าสถานะนั้น" ไม่ใช่ครั้งล่าสุด
 * เพราะถ้าของถูกยิงย้อนแล้วเดินหน้าใหม่ ครั้งล่าสุดจะทำให้ช่วงเวลาติดลบ
 * ส่วนการยิงซ้ำจุดเดิมเพราะไม่แน่ใจว่ายิงติดไหม เป็นเรื่องที่เกิดบ่อยมาก
 *
 * ช่วงเวลาคิดจาก "ขั้นก่อนหน้าที่มีเวลาจริง" ไม่ใช่ขั้นที่ติดกันตามลำดับ
 * เพราะงานจริงข้ามขั้นได้ เช่นจัดเสร็จแล้วขนส่งมารับพอดีเลยไม่ได้ยิงจุดจัดเสร็จ
 * ถ้าคิดจากขั้นที่ติดกันเฉย ๆ ช่วงนั้นจะกลายเป็นว่าง ทั้งที่มีเวลาที่วัดได้อยู่
 *
 * @returns {{steps: Array, firstTs: number, lastTs: number, totalMs: number}}
 */
export function shipTimeline(db, invoice) {
  const events = shipEventsOf(db, invoice.id);

  const firstOf = (statusId) => {
    const hit = events.find((e) => e.status === statusId);
    return hit ? hit.ts : 0;
  };

  // ใบเก่าที่ทำไว้ก่อนมีตารางเหตุการณ์ ยังพอรู้เวลาเริ่มจากตัวใบเอง
  const steps = SHIP_STATUS.map((st) => ({
    id: st.id,
    name: st.name,
    short: st.short,
    color: st.color,
    ts: st.id === "WAIT" ? firstOf("WAIT") || invoice.ts || 0 : firstOf(st.id),
    ms: 0,
  }));
  ...
  steps.forEach((s) => {
    if (!s.ts) return;
    if (prev) s.ms = s.ts - prev;
    prev = s.ts;
  });

  const stamped = steps.filter((s) => s.ts);
  const firstTs = stamped.length ? stamped[0].ts : 0;
  const lastTs = stamped.length ? stamped[stamped.length - 1].ts : 0;

  return { steps, firstTs, lastTs, totalMs: firstTs && lastTs ? lastTs - firstTs : 0 };
}

/** รายการในใบส่งคืนใบหนึ่ง */
export function itemsOfReturn(db, returnId) {
  return (db.purchaseReturnItems || [])
    .filter((i) => i.returnId === returnId)
    .sort((a, b) => (a.seq || 0) - (b.seq || 0));
}

/**
 * คืนไปแล้วกี่หน่วยของบรรทัดใบซื้อนั้น
 *
 * ใช้กันคืนซ้ำเกินจำนวนที่ซื้อมา ฝั่งฐานข้อมูลตรวจซ้ำอีกชั้นใน create_purchase_return
 * แต่ตรวจฝั่งหน้าจอก่อนเพื่อบอกตัวเลขให้เห็นตั้งแต่ตอนกรอก ไม่ใช่ไปเจอตอนกดบันทึก
 */
export function returnedQtyOf(db, purchaseId, itemId) {
  const ids = new Set(
    (db.purchaseReturns || []).filter((r) => r.purchaseId === purchaseId).map((r) => r.id)
  );
  return (db.purchaseReturnItems || [])
    .filter((i) => i.itemId === itemId && ids.has(i.returnId))
    .reduce((sum, i) => sum + (Number(i.qty) || 0), 0);
}

/** ที่อยู่คู่ค้าแบบบรรทัดเดียว ใช้ได้ทั้งลูกค้าและเจ้าหนี้ (โครงข้อมูลเหมือนกัน) */
export const supplierAddress = customerAddress;
...
/**
 * ยอดคงเหลือของสินค้าหนึ่ง แยกรายคลังและรายช่องเก็บ
 *
 * คืนเฉพาะคลังที่เกี่ยวข้องจริง (มียอด หรือมีของวางอยู่ในช่องใดช่องหนึ่ง)
 * เรียงช่องเก็บตามผังคลัง (โซนแล้วคอลัมน์) ไม่ใช่ตามลำดับที่ฐานข้อมูลคืนมา
 * คนที่ถือรายการนี้ไปเดินหยิบของจะได้เดินไล่ตามผังได้เลย
 *
 * loose = ส่วนที่ยอดของคลังมากกว่าผลรวมของทุกช่อง
 *   เกิดกับข้อมูลที่บันทึกไว้ก่อนระบบบังคับให้ระบุช่องเก็บ และกับไฟล์สำรองรุ่นเก่า
 *   ต้องแสดงให้เห็น ไม่ใช่ซ่อนไว้ ไม่งั้นผลรวมของช่องจะไม่เท่ากับยอดของคลัง
 *   แล้วคนอ่านจะคิดว่าตัวเลขใดตัวเลขหนึ่งผิด ทั้งที่ทั้งคู่ถูก แค่ของบางส่วนไม่รู้ว่าอยู่ช่องไหน
 *
 * ติดลบได้ ไม่ได้ปัดให้เป็นศูนย์ — ยอดของคลังน้อยกว่าที่วางไว้ในช่องแปลว่า
 * ข้อมูลสองฝั่งไม่ตรงกัน ซึ่งเป็นเรื่องที่ต้องรู้ ไม่ใช่เรื่องที่ควรกลบ
 */
export function stockByBin(db, stock, productId) {
  return (db.warehouses || [])
    .map((w) => {
      const bins = locsOf(db, w.id)
        .map((l) => ({ loc: l, qty: placedIn(db, productId, l.id) }))
        .filter((b) => b.qty !== 0);
      const placed = bins.reduce((s, b) => s + b.qty, 0);
      const qty = stockOf(stock, productId, w.id);
      // ทศนิยมของจำนวนบวกลบกันแล้วเหลือเศษจิ๋ว ๆ ได้ ปัดทิ้งไม่งั้นจะขึ้นแถว
      // "ไม่ได้ระบุที่เก็บ 0.0000000001" ให้คนอ่านงงเล่น
      const loose = Math.round((qty - placed) * 1e6) / 1e6;
      return { wh: w, qty, bins, placed, loose };
    })
    .filter((x) => x.qty !== 0 || x.bins.length);
}
