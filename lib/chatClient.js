...
/**
 * เช็คว่าผู้ช่วย AI พร้อมใช้หรือยัง (ไม่เรียก Gemini จึงไม่เสียโควตา)
 * @returns {Promise<{configured:boolean, allowed:boolean}>}
 */
export async function chatStatus(signal) {
  let token = null;
  try {
  ...
  if (body && body.error) throw new ChatError(body.error.code, body.error.message);

  return { configured: !!body?.configured, allowed: body?.allowed !== false };
}
