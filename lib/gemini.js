...
// gemini-2.0-flash ถูกปลดระวางไปแล้ว คีย์ใหม่ ๆ เรียกไม่ได้ (ได้ 404)
// เลือกเวอร์ชันแบบระบุเลขไว้ ไม่ใช้ alias -latest เพราะ alias เปลี่ยนรุ่นเองได้
// แล้วพฤติกรรมจะเปลี่ยนโดยเราไม่รู้ตัว
...
export async function callGemini(messages, summary, { timeoutMs = 20000 } = {}) {
  ...
  const payload = JSON.stringify(buildRequestBody(messages, summary, model));
