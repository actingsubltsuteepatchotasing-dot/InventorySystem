...
export async function callGemini(messages, summary, { timeoutMs = 20000 } = {}) {
  ...
  try {
    for (const version of API_VERSIONS) {
      ({ r: res, b: body } = await send(version, {}));

      // บางรุ่นไม่รู้จัก thinkingConfig แล้วตอบ 400 กลับมา ลองใหม่โดยไม่ส่งมัน
      // ดีกว่าปล่อยให้ผู้ใช้เจอ "คำขอไม่ถูกต้อง" ทั้งที่แค่ตัดฟิลด์เดียวออกก็ผ่าน
      const msg400 =
        res.status === 400 && body && body.error ? String(body.error.message || "") : "";
      if (/thinking/i.test(msg400)) {
        ({ r: res, b: body } = await send(version, { noThinking: true }));
      }
