...
const fromVocRecord = (v) => ({
  id: v.id,
  code: v.code,
  // คอลัมน์ชนิดวันที่ห้ามได้สตริงว่าง ไม่งั้น Postgres ฟ้อง invalid input syntax
  // ซึ่งอ่านแล้วไม่รู้ว่าต้องไปแก้ตรงไหน ส่ง null ไปแทนแล้วให้ not null เป็นคนฟ้อง
  date: v.date || null,
...
const toCustomer = (r) => ({
  ...
  taxId: r.tax_id || "",
  branch: r.branch || "",
  // ติ๊กไว้ที่รายไหน หน้าขายสินค้า (POS) จะเลือกรายนั้นให้เองทุกบิล
  posDefault: !!r.pos_default,
});
...
const toSale = (r) => ({
  ...
  whId: r.wh_id,
  locId: r.loc_id || "",
  // รหัสลูกค้าที่เลือกจากทะเบียน ว่าง = ขายให้ลูกค้าทั่วไปที่ไม่ได้อยู่ในทะเบียน
  customerId: r.customer_id || "",
  custCode: r.cust_code || "",
  customer: r.customer || "",
...
const fromQuote = (q) => ({
  ...
  cust_code: q.custCode || "",
  cust_name: q.custName || "",
  cust_address: q.custAddress || "",
  cust_province: q.custProvince || "",
  cust_tax_id: q.custTaxId || "",
  cust_branch: q.custBranch || "",
  cust_kind: q.custKind || "",
  sales_id: q.salesId || null,
  sales_code: q.salesCode || "",
  sales_name: q.salesName || "",
  valid_days: Number(q.validDays) || 0,
  valid_to: q.validTo || null,
  terms: q.terms || "",
  vat_rate: Number(q.vatRate) || 0,
  items_total: Number(q.itemsTotal) || 0,
  bill_discount: Number(q.billDiscount) || 0,
...
const toLineMessage = (r) => ({
  rowOrder: Number(r.row_order) || 0,
  id: r.id,
  msgId: r.msg_id || "",
  source: r.source || "MANUAL",
  sourceKind: r.source_kind || "user",
...
const fromLineMessage = (m) => ({
  id: m.id,
  msg_id: m.msgId || "",
  source: m.source || "MANUAL",
  source_kind: m.sourceKind || "user",
...
const fromLineOrder = (o) => ({
  ...
  note: o.note || "",
  owner: o.owner || "",
  quote_id: o.quoteId || null,
  invoice_id: o.invoiceId || null,
  user_name: o.user || "",
  ts: Number(o.ts) || 0,
});

const toLineOrderItem = (r) => ({
  rowOrder: Number(r.row_order) || 0,
  id: r.id,
  orderId: r.order_id,
  seq: Number(r.seq) || 0,
  raw: r.raw || "",
  // null = อ่านแล้วยังไม่รู้ว่าเป็นสินค้าตัวไหน ต่างจากช่องว่างที่แปลว่าไม่มีข้อมูล
  productId: r.product_id || "",
  qty: Number(r.qty) || 0,
  unit: r.unit || "",
...
const fromLineOrderItem = (i) => ({
  ...
  qty: Number(i.qty) || 0,
  unit: i.unit || "",
  price: i.price === null || i.price === undefined || i.price === "" ? null : Number(i.price),
  matched_by: i.matchedBy || "",
...
export async function loadAll() {
  ...
  // ทะเบียนผู้ใช้และสิทธิรายคน — ระบบที่ยังไม่ได้อัปเดต schema จะไม่มีสองตารางนี้
  // ต้องยังใช้งานได้ตามปกติ โดยถอยไปใช้สิทธิรวมของทั้งระบบเหมือนเดิม
  const [appUsers, userPerms] = await Promise.all([
    fetchOptional("app_users", "email.asc", missing),
    fetchOptional("user_perms", null, missing),
  ]);

  // ใบเสนอราคาและงานผ่านไลน์ — ดึงแยกชุดด้วยเหตุผลเดียวกับหมวดการรับฟังลูกค้า
  // ฐานข้อมูลที่ยังไม่ได้อัปเดต schema จะไม่มีตารางชุดนี้ ส่วนอื่นต้องยังใช้งานได้ตามปกติ
  const [quotes, quoteItems, lineMessages, lineOrders, lineOrderItems, lineAliases] =
    await Promise.all([
      fetchOptional("quotes", "ts.asc", missing),
      fetchOptional("quote_items", null, missing),
      fetchOptional("line_messages", "ts.asc", missing),
      fetchOptional("line_orders", "ts.asc", missing),
      fetchOptional("line_order_items", null, missing),
      fetchOptional("line_aliases", "word.asc", missing),
    ]);
  ...
  return {
    ...
    appUsers: (appUsers || []).map(toAppUser),
    userPerms: (userPerms || []).map(toUserPerm),
    quotes: (quotes || []).map(toQuote).sort((a, b) => a.ts - b.ts),
    quoteItems: (quoteItems || []).map(toQuoteItem),
    lineMessages: (lineMessages || []).map(toLineMessage).sort((a, b) => a.ts - b.ts),
    lineOrders: (lineOrders || []).map(toLineOrder).sort((a, b) => a.ts - b.ts),
    lineOrderItems: (lineOrderItems || []).map(toLineOrderItem),
    lineAliases: (lineAliases || []).map(toLineAlias),
...
export async function healthCheck() {
  const TABLES = [
    ...
    { name: "quotes", label: "ตารางใบเสนอราคา" },
    { name: "quote_items", label: "ตารางรายการในใบเสนอราคา" },
    { name: "line_messages", label: "ตารางข้อความจากไลน์" },
    { name: "line_orders", label: "ตารางคำสั่งซื้อจากไลน์" },
    { name: "line_order_items", label: "ตารางรายการในคำสั่งซื้อจากไลน์" },
    { name: "line_aliases", label: "ตารางคำเรียกสินค้าของลูกค้า" },
    ...
    { name: "app_users", label: "ตารางผู้ใช้ในระบบ" },
    { name: "user_perms", label: "ตารางสิทธิรายผู้ใช้" },
  ];
...
/**
 * หน้าจอกำหนดสิทธิรายผู้ใช้พร้อมใช้หรือยัง
 *
 * ต้องมีครบทั้งสองตาราง เพราะสิทธิรายคนอ้างทะเบียนผู้ใช้
 * ไม่ครบแล้วเปิดใช้ จะตั้งสิทธิให้คนที่ไม่มีในทะเบียนได้ แล้วสิทธินั้นจะไม่มีผลกับใคร
 */
export const usersReady = (missing) =>
  !missing.includes("app_users") && !missing.includes("user_perms");

/**
 * ลงทะเบียนตัวเองเข้าทะเบียนผู้ใช้ ถ้ายังไม่มี
 *
 * เรียกตอนโหลดข้อมูลเสร็จ ทำงานเงียบ ๆ ไม่ขวางการใช้งานถ้าทำไม่สำเร็จ
 *
 * ทำไมต้องมีทั้งที่ฐานข้อมูลมี trigger รับผู้ใช้ใหม่อยู่แล้ว:
 *   บางโปรเจกต์สร้าง trigger บน schema auth ไม่ได้เพราะสิทธิ์ถูกจำกัด
 *   ถ้าพึ่ง trigger อย่างเดียว ผู้ใช้จะไม่โผล่ในทะเบียนและแอดมินจะตั้งสิทธิให้ไม่ได้เลย
 *   ทางนี้เป็นทางสำรองที่ไม่ต้องแตะ schema auth
 *
 * policy ของฐานข้อมูลยอมให้เพิ่มได้เฉพาะแถวของตัวเองและเป็นบทบาท user เท่านั้น
 * จึงยกบทบาทตัวเองเป็นแอดมินด้วยวิธีนี้ไม่ได้
 */
export async function ensureAppUser(user) {
  if (!user || !user.id) return false;
  try {
    await rest("app_users?on_conflict=id", {
      method: "POST",
      headers: { Prefer: "return=minimal,resolution=ignore-duplicates" },
      body: JSON.stringify([
        { id: user.id, email: user.email || "", role: "user", ts: Date.now() },
      ]),
    });
    return true;
  } catch (e) {
    // ตารางยังไม่มี หรือ policy ไม่ยอม — ไม่ใช่เรื่องที่ต้องขวางการใช้งาน
    return false;
  }
}

/** แก้ไขบทบาท ชื่อ หรือสถานะของผู้ใช้ — ฐานข้อมูลยอมเฉพาะแอดมิน */
export async function upsertAppUser(u) {
  await rest("app_users?on_conflict=id", {
    method: "POST",
    headers: { Prefer: "return=minimal,resolution=merge-duplicates" },
    body: JSON.stringify([fromAppUser(u)]),
  });
}

/**
 * ลบผู้ใช้ออกจากทะเบียน — สิทธิรายคนของคนนั้นถูกลบตามไปด้วย (cascade)
 *
 * ลบที่นี่ไม่ได้ลบบัญชีเข้าระบบ บัญชียังอยู่ใน auth.users และยังล็อกอินได้
 * ถ้าต้องการปิดไม่ให้ใช้งาน ให้ปิดใช้งานแทนการลบ
 */
export async function deleteAppUser(id) {
  await rest("app_users?id=eq." + encodeURIComponent(id), {
    method: "DELETE",
    headers: { Prefer: "return=minimal" },
  });
}

/** ตั้งสิทธิของผู้ใช้หนึ่งคนกับหน้าจอหนึ่ง */
export async function upsertUserPerm(p) {
  await rest("user_perms?on_conflict=id", {
    method: "POST",
    headers: { Prefer: "return=minimal,resolution=merge-duplicates" },
    body: JSON.stringify([fromUserPerm(p)]),
  });
}

/** ตั้งสิทธิหลายหน้าจอพร้อมกัน — ใช้ตอนกดบันทึกทั้งหน้า */
export async function saveUserPerms(list) {
  if (!list.length) return;
  await rest("user_perms?on_conflict=id", {
    method: "POST",
    headers: { Prefer: "return=minimal,resolution=merge-duplicates" },
    body: JSON.stringify(list.map(fromUserPerm)),
  });
}

/** ล้างสิทธิเฉพาะตัวของคนหนึ่ง กลับไปใช้ค่าเริ่มต้นของทุกคน */
export async function clearUserPerms(userId) {
  await rest("user_perms?user_id=eq." + encodeURIComponent(userId), {
    method: "DELETE",
    headers: { Prefer: "return=minimal" },
  });
}
...
/** ฟีเจอร์ใบเสนอราคาพร้อมใช้หรือยัง */
export const quotesReady = (missing) =>
  !missing.includes("quotes") && !missing.includes("quote_items");

/**
 * ฟีเจอร์งานผ่านไลน์พร้อมใช้หรือยัง
 *
 * ต้องครบทั้งสี่ตาราง เพราะขาดตารางไหนไปก็ใช้งานไม่ได้จริง:
 * ข้อความที่แปลงเป็นคำสั่งซื้อไม่ได้ หรือคำสั่งซื้อที่ไม่มีรายการสินค้า
 * ล้วนเป็นหน้าจอที่เปิดมาแล้วทำอะไรต่อไม่ได้ ซึ่งแย่กว่าบอกตรง ๆ ว่ายังไม่พร้อม
 */
export const lineReady = (missing) =>
  !missing.includes("line_messages") &&
  !missing.includes("line_orders") &&
  !missing.includes("line_order_items") &&
  !missing.includes("line_aliases");

/* -------------------------------------------------------- ใบเสนอราคา */

/** เพิ่ม/แก้ไขใบเสนอราคาพร้อมรายการ — ลบรายการเดิมทิ้งแล้วใส่ชุดใหม่ */
export async function upsertQuote(quote, items) {
  await rest("quotes", {
    method: "POST",
    headers: { Prefer: "return=minimal,resolution=merge-duplicates" },
    body: JSON.stringify([fromQuote(quote)]),
  });

  /*
   * ลบรายการเดิมทั้งชุดแล้วใส่ใหม่ ไม่ได้ไล่เทียบทีละบรรทัด
   * เพราะบรรทัดในใบเสนอราคาไม่มีความหมายข้ามครั้งแก้ (ไม่มีอะไรอ้างถึงมัน
   * และไม่ได้ขยับสต็อก) การไล่เทียบจึงเพิ่มความซับซ้อนโดยไม่ได้อะไรกลับมา
   */
  await rest("quote_items?quote_id=eq." + encodeURIComponent(quote.id), {
    method: "DELETE",
    headers: { Prefer: "return=minimal" },
  });
  ...
  if (items && items.length) {
    await rest("quote_items", {
      method: "POST",
      headers: { Prefer: "return=minimal" },
      body: JSON.stringify(items.map(fromQuoteItem)),
    });
  }
}

/** ลบใบเสนอราคา — รายการในใบถูกลบตามด้วย cascade */
export async function deleteQuote(id) {
  await rest("quotes?id=eq." + encodeURIComponent(id), {
    method: "DELETE",
    headers: { Prefer: "return=minimal" },
  });
}
...
/** บันทึกข้อความจากไลน์หลายข้อความพร้อมกัน (ใช้ตอนนำเข้าไฟล์แชท) */
export async function insertLineMessages(list) {
  if (!list || !list.length) return;
  /*
   * insertChunked ใช้ merge-duplicates อยู่แล้ว โหลดไฟล์เดิมซ้ำจึงไม่ได้ข้อความซ้ำ
   * เพราะ id ของข้อความคิดจากเนื้อหา + เวลา + ผู้ส่ง (ดู msgKey ในหน้าจอ)
   * คนโหลดไฟล์ซ้ำเป็นเรื่องปกติมาก โดยเฉพาะตอนไม่แน่ใจว่าโหลดไปแล้วหรือยัง
   */
  await insertChunked("line_messages", list.map(fromLineMessage));
}

/** เพิ่ม/แก้ไขข้อความจากไลน์หนึ่งข้อความ */
export async function upsertLineMessage(m) {
  await rest("line_messages", {
    method: "POST",
    headers: { Prefer: "return=minimal,resolution=merge-duplicates" },
    body: JSON.stringify([fromLineMessage(m)]),
  });
}

/** ลบข้อความจากไลน์ */
export async function deleteLineMessage(id) {
  await rest("line_messages?id=eq." + encodeURIComponent(id), {
    method: "DELETE",
    headers: { Prefer: "return=minimal" },
  });
}

/** เพิ่ม/แก้ไขคำสั่งซื้อจากไลน์พร้อมรายการ — กติกาเดียวกับใบเสนอราคา */
export async function upsertLineOrder(order, items) {
  await rest("line_orders", {
    method: "POST",
    headers: { Prefer: "return=minimal,resolution=merge-duplicates" },
    body: JSON.stringify([fromLineOrder(order)]),
  });

  await rest("line_order_items?order_id=eq." + encodeURIComponent(order.id), {
    method: "DELETE",
    headers: { Prefer: "return=minimal" },
  });

  if (items && items.length) {
    await rest("line_order_items", {
      method: "POST",
      headers: { Prefer: "return=minimal" },
      body: JSON.stringify(items.map(fromLineOrderItem)),
    });
  }
}

/** ลบคำสั่งซื้อจากไลน์ — รายการในคำสั่งซื้อถูกลบตามด้วย cascade */
export async function deleteLineOrder(id) {
  await rest("line_orders?id=eq." + encodeURIComponent(id), {
    method: "DELETE",
    headers: { Prefer: "return=minimal" },
  });
}

/** ผูกข้อความเข้ากับคำสั่งซื้อ (หรือปลดออกเมื่อส่ง orderId เป็นค่าว่าง) */
export async function linkLineMessages(ids, orderId) {
  if (!ids || !ids.length) return;
  const list = ids.map((x) => '"' + x + '"').join(",");
  await rest("line_messages?id=in.(" + encodeURIComponent(list) + ")", {
    method: "PATCH",
    headers: { Prefer: "return=minimal" },
...
/** เพิ่ม/แก้ไขคำเรียกสินค้าของลูกค้า */
export async function upsertLineAlias(a) {
  await rest("line_aliases", {
    method: "POST",
    headers: { Prefer: "return=minimal,resolution=merge-duplicates" },
    body: JSON.stringify([fromLineAlias(a)]),
  });
}

/** ลบคำเรียกสินค้าของลูกค้า */
export async function deleteLineAlias(id) {
  await rest("line_aliases?id=eq." + encodeURIComponent(id), {
    method: "DELETE",
    headers: { Prefer: "return=minimal" },
  });
}
...
export async function upsertCustomer(customer) {
  /*
   * ลูกค้าเริ่มต้นของหน้า POS มีได้ทีละรายเดียว ปลดของรายอื่นก่อนเสมอ
   * (วิธีเดียวกับฟอร์มพิมพ์ที่เป็นค่าเริ่มต้น ดู upsertPrintForm)
   *
   * ปลดก่อนแล้วค่อยบันทึก ไม่ใช่กลับกัน — ถ้าบันทึกก่อนแล้วปลดทีหลังพลาด
   * จะเหลือสองรายที่ติ๊กไว้พร้อมกัน ส่วนลำดับนี้แย่ที่สุดคือไม่มีรายไหนเป็นค่าเริ่มต้น
   * ซึ่งกดตั้งใหม่ได้ทันทีและไม่ทำให้ข้อมูลขัดกันเอง
   */
  if (customer.posDefault) {
    await rest(
      "customers?id=neq." + encodeURIComponent(customer.id) + "&pos_default=is.true",
      {
        method: "PATCH",
        headers: { Prefer: "return=minimal" },
        body: JSON.stringify({ pos_default: false }),
      }
    );
  }
...
async function clearAll() {
  ...
  // สิทธิรายคนต้องลบก่อนทะเบียนผู้ใช้ เพราะอ้างถึงกัน
  // งานผ่านไลน์: รายการ -> คำสั่งซื้อ -> ข้อความ -> คำเรียก
  // และใบเสนอราคาต้องลบก่อนคำสั่งซื้อ เพราะคำสั่งซื้ออ้างถึงใบ
  await skipIfMissing(rest("line_order_items?id=not.is.null", opts));
  await skipIfMissing(rest("line_messages?id=not.is.null", opts));
  await skipIfMissing(rest("line_orders?id=not.is.null", opts));
  await skipIfMissing(rest("line_aliases?id=not.is.null", opts));
  await skipIfMissing(rest("quote_items?id=not.is.null", opts));
  await skipIfMissing(rest("quotes?id=not.is.null", opts));
  await skipIfMissing(rest("user_perms?id=not.is.null", opts));
  await skipIfMissing(rest("app_users?id=not.is.null", opts));
...
export async function replaceAll(data) {
  ...
  // ลำดับสำคัญ: ใบเสนอราคาต้องมาก่อนคำสั่งซื้อจากไลน์ เพราะคำสั่งซื้ออ้างถึงใบ
  // และคำสั่งซื้อต้องมาก่อนทั้งรายการในคำสั่งซื้อและข้อความที่ผูกกับมัน
  await skipIfMissing(insertChunked("quotes", (data.quotes || []).map(fromQuote)));
  await skipIfMissing(insertChunked("quote_items", (data.quoteItems || []).map(fromQuoteItem)));
  await skipIfMissing(insertChunked("line_aliases", (data.lineAliases || []).map(fromLineAlias)));
  await skipIfMissing(insertChunked("line_orders", (data.lineOrders || []).map(fromLineOrder)));
  await skipIfMissing(
    insertChunked("line_order_items", (data.lineOrderItems || []).map(fromLineOrderItem))
  );
  await skipIfMissing(
    insertChunked("line_messages", (data.lineMessages || []).map(fromLineMessage))
  );

  // ทะเบียนผู้ใช้ต้องมาก่อนสิทธิรายคน เพราะสิทธิอ้างถึงผู้ใช้
  await skipIfMissing(insertChunked("app_users", (data.appUsers || []).map(fromAppUser)));
  await skipIfMissing(insertChunked("user_perms", (data.userPerms || []).map(fromUserPerm)));
}
...
export async function seedIfEmpty(current) {
  ...
  // ข้อมูลตั้งต้นไม่มีใบเสนอราคาและงานไลน์ แต่ยังต้องบอกหน้าจอว่าตารางพร้อมหรือยัง
  if (!quotesReady(missing)) {
    fresh.quotes = [];
    fresh.quoteItems = [];
  }
  if (!lineReady(missing)) {
    fresh.lineMessages = [];
    fresh.lineOrders = [];
    fresh.lineOrderItems = [];
    fresh.lineAliases = [];
  }
