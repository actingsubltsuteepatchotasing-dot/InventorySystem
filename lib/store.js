...
export function InventoryProvider({ children }) {
  ...
  const saveCustomer = useCallback(async (customer) => {
    ...
    setDb((prev) => {
      if (!prev) return prev;
      // ตั้งเป็นลูกค้าเริ่มต้นของหน้า POS ได้ทีละรายเดียว รายอื่นถูกปลดให้เอง
      // ฝั่งฐานข้อมูลก็ปลดให้เหมือนกัน (upsertCustomer) ตรงนี้ทำเพื่อให้หน้าจอ
      // เปลี่ยนตามทันทีโดยไม่ต้องรอโหลดข้อมูลใหม่ทั้งชุด
      const list = (prev.customers || []).map((c) =>
        customer.posDefault && c.id !== customer.id && c.posDefault
          ? { ...c, posDefault: false }
          : c
      );
  ...
  const saveQuote = useCallback(async (quote, items) => {
    await api.upsertQuote(quote, items);
    setDb((prev) =>
      prev
        ? {
            ...prev,
            quotes: [...(prev.quotes || []).filter((x) => x.id !== quote.id), quote].sort(
              (a, b) => a.ts - b.ts
            ),
            // รายการของใบนี้ถูกเขียนทับทั้งชุด จึงต้องเอาของเดิมออกก่อน
            // ไม่งั้นบรรทัดที่ถูกลบไปแล้วจะยังค้างอยู่บนหน้าจอจนกว่าจะโหลดใหม่
            quoteItems: [
              ...(prev.quoteItems || []).filter((x) => x.quoteId !== quote.id),
              ...(items || []),
            ],
          }
        : prev
    ...
    setDb((prev) =>
      prev
        ? {
            ...
            quoteItems: (prev.quoteItems || []).filter((x) => x.quoteId !== id),
          }
        : prev
    );
  }, []);
  ...
  const saveLineMessages = useCallback(async (list) => {
    await api.insertLineMessages(list);
    setDb((prev) => {
      if (!prev) return prev;
      const ids = new Set(list.map((m) => m.id));
      return {
        ...prev,
        lineMessages: [...(prev.lineMessages || []).filter((m) => !ids.has(m.id)), ...list].sort(
          (a, b) => a.ts - b.ts
        ),
      };
    });
  ...
  const removeLineMessage = useCallback(async (id) => {
    await api.deleteLineMessage(id);
    setDb((prev) =>
      prev
        ? { ...prev, lineMessages: (prev.lineMessages || []).filter((m) => m.id !== id) }
        : prev
    );
  }, []);

  const saveLineOrder = useCallback(async (order, items) => {
    await api.upsertLineOrder(order, items);
    setDb((prev) =>
      prev
        ? {
            ...prev,
            lineOrders: [...(prev.lineOrders || []).filter((x) => x.id !== order.id), order].sort(
              (a, b) => a.ts - b.ts
            ),
            lineOrderItems: [
              ...(prev.lineOrderItems || []).filter((x) => x.orderId !== order.id),
              ...(items || []),
            ],
          }
        : prev
  ...
  const removeLineOrder = useCallback(async (id) => {
    await api.deleteLineOrder(id);
    setDb((prev) =>
      prev
        ? {
            ...prev,
            lineOrders: (prev.lineOrders || []).filter((x) => x.id !== id),
            lineOrderItems: (prev.lineOrderItems || []).filter((x) => x.orderId !== id),
            // ข้อความที่เคยผูกกับคำสั่งซื้อนี้ต้องกลับไปเป็นข้อความที่ยังไม่ได้จัดการ
            // ฐานข้อมูลไม่ได้ตั้ง cascade ไว้ เพราะข้อความเป็นหลักฐาน ห้ามหายตามไปด้วย
            lineMessages: (prev.lineMessages || []).map((m) =>
              m.orderId === id ? { ...m, orderId: "" } : m
            ),
          }
        : prev
    );
  ...
  const linkLineMessages = useCallback(async (ids, orderId) => {
    await api.linkLineMessages(ids, orderId);
    setDb((prev) => {
      if (!prev) return prev;
      ...
      return {
        ...prev,
        lineMessages: (prev.lineMessages || []).map((m) =>
          set.has(m.id) ? { ...m, orderId: orderId || "" } : m
        ),
      };
    });
  }, []);

  const saveLineAlias = useCallback(async (a) => {
    await api.upsertLineAlias(a);
    setDb((prev) =>
      prev
        ? {
            ...prev,
            lineAliases: [...(prev.lineAliases || []).filter((x) => x.id !== a.id), a].sort(
              (x, y) => x.word.localeCompare(y.word, "th")
            ),
          }
        : prev
    );
  ...
  const removeLineAlias = useCallback(async (id) => {
    await api.deleteLineAlias(id);
    setDb((prev) =>
      prev ? { ...prev, lineAliases: (prev.lineAliases || []).filter((x) => x.id !== id) } : prev
    );
  }, []);
  ...
  const value = useMemo(
    () => ({
      ...
      // ยอดคงเหลือของสินค้าหนึ่ง แยกรายคลังและรายช่องเก็บ (หน้าข้อมูลสินค้าใช้)
      stockByBin: (pid) => (db ? stockByBin(db, stock, pid) : []),
      ...
      quotesReady: api.quotesReady(missingTables),
      lineReady: api.lineReady(missingTables),
      saveVocChannel,
      removeVocChannel,
      saveVocRecord,
      removeVocRecord,
    ...
    [
      ...
      saveQuote, removeQuote, saveLineMessages, removeLineMessage,
      saveLineOrder, removeLineOrder, linkLineMessages, saveLineAlias, removeLineAlias,
