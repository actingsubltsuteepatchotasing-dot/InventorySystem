...
export async function rest(path, options = {}) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) {
      throw new Error("เซสชันหมดอายุ กรุณาเข้าสู่ระบบใหม่" + (raw ? " [" + raw + "]" : ""));
    }
    ...
    if (res.status === 403 || info.code === "42501") {
      ...
      throw new Error(
        "สิทธิ์ระดับตารางไม่พอ — role authenticated ยังไม่ได้รับ GRANT บนตารางนี้ " +
          "ให้รัน supabase/fix-permissions.sql ใน SQL Editor [" + raw + "]"
      );
    }

    if (res.status === 404 || info.code === "42P01" || info.code === "PGRST205") {
      throw new Error(
        "ไม่พบตารางในฐานข้อมูล — ยังไม่ได้รัน supabase/schema.sql หรือ Data API " +
          "ยังไม่ได้เปิด schema public [" + raw + "]"
      );
    }

    throw new Error(raw || "เรียกฐานข้อมูลไม่สำเร็จ (HTTP " + res.status + ")");
  }
