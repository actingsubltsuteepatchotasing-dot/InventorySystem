...
/**
 * วันที่ของ Date ตามเขตเวลาเครื่องผู้ใช้ แบบ YYYY-MM-DD
 *
 * ห้ามใช้ toISOString() เพราะมันเป็นเวลา UTC
 * ไทยเป็น UTC+7 ตั้งแต่เที่ยงคืนถึงเจ็ดโมงเช้าจึงได้วันที่ของ "เมื่อวาน" กลับมา
 * เอกสารที่บันทึกตอนกะดึกจะลงวันผิดไปทั้งใบโดยไม่มีใครสังเกต
 */
...
/** เวลาแบบสั้น ใช้ในตารางที่มีหลายคอลัมน์เวลา */
export function thTime(ts) {
  return new Date(ts).toLocaleString("th-TH", {
    day: "2-digit",
    month: "2-digit",
    hour: "2-digit",
...
/**
 * ระยะเวลาแบบที่คนอ่านแล้วเข้าใจทันที เช่น "2 ชม. 15 นาที"
 *
 * ไม่แสดงเป็นทศนิยมของชั่วโมง เพราะ "1.75 ชม." ต้องคิดในหัวอีกทีว่ากี่นาที
 * และไม่ลงลึกกว่านาที เพราะงานคลังไม่มีใครสนใจว่าต่างกันกี่วินาที
 */
export function fmtDuration(ms) {
  if (!ms || ms < 0) return "—";
  // ต้องเช็คจากมิลลิวินาทีก่อนปัดเศษ ไม่งั้น 30 วินาทีจะถูกปัดขึ้นเป็น "1 นาที"
  // ซึ่งอ่านแล้วเข้าใจว่าใช้เวลาหนึ่งนาทีจริง ทั้งที่ยังไม่ถึง
  if (ms < 60000) return "ไม่ถึง 1 นาที";
  ...
  const rest = mins % 60;
  if (hours < 24) return hours + " ชม." + (rest ? " " + rest + " นาที" : "");

  const days = Math.floor(hours / 24);
  const hRest = hours % 24;
  return days + " วัน" + (hRest ? " " + hRest + " ชม." : "");
}
 ...
 */
/**
 * จำนวนวันระหว่างวันที่สองค่าแบบ YYYY-MM-DD (ปลายทางลบต้นทาง)
 *
 * แตกตัวเลขจากสตริงเองแล้วประกอบด้วย Date.UTC ไม่ได้ยัดสตริงเข้า new Date() ตรง ๆ
 * เพราะ new Date("2026-09-06") อ่านเป็นเวลา UTC ส่วน new Date("2026/09/06")
 * อ่านเป็นเวลาท้องถิ่น ผลต่างจึงเพี้ยนไปหนึ่งวันในบางกรณี
 * ใช้ UTC ทั้งสองฝั่งเหมือนกัน ผลต่างของ "วัน" จึงไม่มีเรื่องเขตเวลามาเกี่ยว
 *
 * วันที่ไม่ถูกรูปแบบคืน 0 ไม่ใช่ NaN เพราะ NaN จะไหลไปโผล่บนหน้าจอเป็น "NaN วัน"
 */
