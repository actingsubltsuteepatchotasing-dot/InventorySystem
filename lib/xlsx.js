...
function drawingXML(shapes) {
  ...
  const body = (sp, i) => {
    ...
    /*
     * ข้อความแยกเป็นหลายย่อหน้าตามตัวขึ้นบรรทัด
     * ต้องแตกเองเพราะ DrawingML ไม่มีอักขระขึ้นบรรทัดในข้อความ
     * ใส่ \n ดิบลงไปแล้ว Excel จะบอกว่าไฟล์เสียหาย
     */
    const paras = String(sp.text || "")
      .split("\n")
      .map(
        (t) =>
          '<a:p><a:pPr algn="' + (sp.align || "ctr") + '"/>' +
          (t
            ? '<a:r><a:rPr lang="th-TH" sz="' + (sp.size || 1000) + '"' +
              (sp.bold ? ' b="1"' : "") +
              '><a:solidFill><a:srgbClr val="' + (sp.color || "1B3D2A") + '"/></a:solidFill>' +
              '<a:latin typeface="Tahoma"/><a:cs typeface="Tahoma"/></a:rPr>' +
              "<a:t>" + esc(t) + "</a:t></a:r>"
            : "") +
          "</a:p>"
      )
